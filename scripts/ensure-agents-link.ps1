...
$targetRelative = ".codex\AGENTS.md"
...
if (-not (Test-Path $targetPath)) {
    throw "Expected AgentsCompany instructions at $targetPath. Initialize the .codex submodule first."
}
...
if (Test-Path $linkPath) {
    ...
    $hasCanonicalTarget = $existingTargets -contains $targetRelative -or $existingTargets -contains ".codex/AGENTS.md"
    ...
    if ($hasCanonicalTarget -or $hasResolvedTarget) {
        if ($existing.LinkType -eq "SymbolicLink") {
            if ($hasCanonicalTarget) {
                ...
                Write-WarnLine "Git tracks a portable relative symlink; rerun this script in a symlink-capable shell when convenient."
            }
        } elseif ($existing.LinkType -eq "HardLink") {
            & $git update-index --skip-worktree AGENTS.md
...
try {
    ...
    try {
        ...
        Write-WarnLine "To upgrade it later, rerun this script from an elevated terminal or with Developer Mode enabled."
        exit 0
    } catch {
        $manual = @(
            "Manual fix required.",
            ...
            "3. Run: New-Item -ItemType SymbolicLink -Path AGENTS.md -Target .codex\AGENTS.md",
